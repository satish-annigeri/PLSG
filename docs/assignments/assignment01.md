...
8. What is the output of `#!python len((1, 2, (3, 4)))`?<br>i) `3` ii) `4` iii) `2` iv) Error
9. Which of the following data type is mutable?<br>i) `int` ii) `str` iii) `tuple` iv) `list`
10. What is the result of `#!python ('a', 'b') + ('c',)`?<br>i) Error ii) `#!python ['a', 'b', 'c']` iii) `#!python ('a', 'b', 'c')` iv) `#!python ('a', 'b')`
